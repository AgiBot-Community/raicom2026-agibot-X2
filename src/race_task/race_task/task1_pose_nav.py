...
def forced_direction_yaw_error(target_yaw, current_yaw, direction):
    """
    Return yaw error for forced final turning direction.

    direction == "right":
        Always use clockwise/right turn command.
        In ROS / cmd_vel convention, right turn is usually angular.z < 0.

    direction == "left":
        Always use counter-clockwise/left turn command.
        In ROS / cmd_vel convention, left turn is usually angular.z > 0.

    Note:
        This function is only used for control command in FINAL_ALIGN.
        Stop condition still uses shortest yaw error, so the robot stops when
        the target yaw is reached, instead of spinning a full circle after crossing it.
    """
    shortest_error = normalize_angle(target_yaw - current_yaw)

    if direction == "left":
        if shortest_error >= 0.0:
            return shortest_error
        return shortest_error + 2.0 * math.pi
...
class Task1PoseNavigator(Node):
    def __init__(self):
        super().__init__("task1_pose_nav")
        ...
        # 新增：最终转身阶段固定方向。
        # 默认 right，即 FINAL_ALIGN 阶段固定右转。
        # 如果现场发现正负方向和实际相反，可以启动时传：
        #   -p final_align_turn_direction:=left
        self.final_align_turn_direction = str(
            self.declare_parameter("final_align_turn_direction", "left").value
        ).lower()

        if self.final_align_turn_direction not in ["right", "left"]:
            self.get_logger().warn(
                f"Invalid final_align_turn_direction={self.final_align_turn_direction}, "
                "fallback to left."
            )
            self.final_align_turn_direction = "left"
        ...
        self.get_logger().info("task1_pose_nav started.")
        ...
        self.get_logger().info(
            f"Final align turn direction: {self.final_align_turn_direction}"
        )

        if self.wait_for_goal_from_rviz:
            self.get_logger().warn(
                f"Waiting for external PoseStamped goal on {self.goal_pose_topic}. "
                "The coordinator will publish the target when Task 1 starts."
            )
        else:
            self.get_logger().info(
    ...
    def publish_reached_once(self):
        """Publish one completion event for the current Task 1 goal."""
    ...
    def control_loop(self):
        ...
        # 这个仍然是“最短 yaw 误差”，只用于判断是否已经到达最终朝向。
        final_yaw_error = normalize_angle(self.target_yaw - yaw)

        # 这个是“强制方向 yaw 误差”，只用于 FINAL_ALIGN 阶段发角速度。
        # 默认固定右转。
        forced_final_yaw_error = forced_direction_yaw_error(
            self.target_yaw,
            yaw,
            self.final_align_turn_direction,
        )
        ...
        if self.state == NavState.ALIGN_TO_GOAL:
            ...
            # 到达判断仍然用最短误差，避免转过目标角后还继续转一整圈。
            if abs(final_yaw_error) <= self.goal_yaw_tolerance:
                self.publish_stop()
                self.state = NavState.DONE
                self.publish_reached_once()
                self.get_logger().info("Task 1 target reached. Robot stopped.")
                self.goal_received = False
                self.state = NavState.WAIT_GOAL
                return

            # 关键改动：
            # 原来这里用 raw_w = k_angular * final_yaw_error
            # 它会自动选择最短方向。
            #
            # 现在改成 forced_final_yaw_error：
            # final_align_turn_direction=right 时，forced_final_yaw_error 永远 <= 0，
            # 所以 FINAL_ALIGN 阶段永远右转。
            raw_w = self.k_angular * forced_final_yaw_error
            cmd.angular.z = apply_min_threshold(
                raw_w,
                self.min_angular_velocity,
                self.max_angular_velocity,
        ...
        self.get_logger().info(
            ...
            f"forced_final_err={forced_final_yaw_error:.2f}, "
            f"turn_dir={self.final_align_turn_direction}, "
