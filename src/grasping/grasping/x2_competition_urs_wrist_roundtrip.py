...
class CompetitionUrsWristRoundtrip(SafeUrsWristTest):
    def publish_upper(
        self,
        arm_position,
        head_position,
        ...
        message.header = MessageHeader()
        message.header.stamp.sec = now.nanoseconds // 1_000_000_000
        message.header.stamp.nanosec = now.nanoseconds % 1_000_000_000
        message.header.frame_id = "mc_upper_body"
        message.header.sequence = self.sequence
        self.sequence += 1
        message.source = "upper_body_example"

        message.hand_sub_mode = UpperBodyCommandArray.HAND_CLAW_OPEN_CLOSE
        message.hand_pos = [float(value) for value in hand_position]
        message.head_pos = [float(value) for value in head_position]
        message.arm_pos = [float(value) for value in arm_position]
    ...
    def publish_segment(
        ...
        while rclpy.ok() and time.monotonic() < deadline:
            now = time.monotonic()
            alpha = min(1.0, max(0.0, (now - start_time) / duration_sec))
            command_arm = [
                float(start + (goal - start) * alpha)
                for start, goal in zip(start_arm, target_arm)
            ]
            self.publish_upper(command_arm, head_position, hand_position)
    ...
    def set_mode_while_holding(
        ...
        if not self.mode_client.wait_for_service(timeout_sec=5.0):
            raise RuntimeError(f"模式服务不可用：{MODE_SERVICE}")
        ...
        while rclpy.ok() and not future.done() and time.monotonic() < deadline:
            self.publish_upper(hold_arm, hold_head, hold_hand)
        ...
        if not future.done():
            raise RuntimeError(f"切换模式超时：{action_name}")

        response = future.result()
        if response is None:
            raise RuntimeError(f"模式切换返回空响应：{action_name}")

        status = int(response.response.status.value)
        if status != int(CommonState.SUCCESS):
            raise RuntimeError(f"模式切换失败：{action_name}")
