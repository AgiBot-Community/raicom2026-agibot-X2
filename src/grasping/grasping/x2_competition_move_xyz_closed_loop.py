...
class XYZClosedLoopNode(
    ...
    def __init__(self):
        ...
        # 每个worker/node实例只允许发送一次最终成功状态。
        # 真正发送时机由抓取worker在“闭爪 + 举起完成”后触发。
        self._grasp_success_sent = False

    def publish_grasp_success(self):
        """发布最终抓取成功状态。

        这里只提供发布接口，不在XYZ运动过程中自动调用。
        应由 x2_grasp_worker.py 在 POST_GRASP_LIFT_COMPLETE 后调用。
        """

        if self._grasp_success_sent:
            self.get_logger().warning(
                "GRASP_STATUS_SUCCESS_ALREADY_SENT=true"
            )
            return
        ...
        self._grasp_success_sent = True

        self.get_logger().info(
            "GRASP_STATUS_PUBLISHED=success"
        )

    def publish_grasp_failure(self, reason=""):
        """可选：发布抓取失败状态。"""
        ...
        if reason:
            self.get_logger().error(
                f"GRASP_STATUS_PUBLISHED=failed | REASON={reason}"
            )
        else:
            self.get_logger().error(
                "GRASP_STATUS_PUBLISHED=failed"
            )
